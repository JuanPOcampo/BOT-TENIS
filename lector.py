...
def buscar_precio_referencia(referencia, inventario):
    for item in inventario:
        if item.get('modelo', '').strip().lower() == referencia.strip().lower():
...
async def responder(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # 4) Reinicio explícito (/start, inicio, etc.)
    ...
    # 5) Intención global de enviar imagen (en cualquier fase)
...
async def responder(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Luego ya sigues con tu flujo normal
    fase = est['fase']

    if fase == 'esperando_comando':
        pass
